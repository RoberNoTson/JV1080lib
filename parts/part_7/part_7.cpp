...
void JVlibForm::on_Part7_PatchGroup_select_currentIndexChanged(int val) {
 ...
 if (state_table->updates_enabled) {
  ...
  // update JV
  if (state_table->updates_enabled) {
    change_3(0xB0 + Part7_MidiChannel_select->value()-1, 0, MSB);
    change_3(0xB0 + Part7_MidiChannel_select->value()-1, 0x20, LSB);
    // Program Change - patch number is required for this
    change_2(0xC0 + Part7_MidiChannel_select->value()-1, Lval);
...
void JVlibForm::on_Part7_PatchNumber_select_valueChanged(int val) {
  ...
  if (state_table->updates_enabled) {
    ...
    if (state_table->jv_connect)
      change_2(0xC0 + Part7_MidiChannel_select->value()-1, pn);
    if (state_table->perf_mode) 
      if (CtlChl) SysControlRecvChannel_select->setValue(CtlChl);
    Part7_PatchName_display->setText(getPartPatchName(6));
  }	// end updates_enabled
