...
void JVlibForm::on_Part3_Pan_select_valueChanged(int val) {
  Part3_Level_select->setStatusTip(QString::number(val));
  if (state_table->perf_mode) setPartSingleValue(2,7,val);
  if (state_table->GM_mode && state_table->updates_enabled) change_3(0xB0+Part3_MidiChannel_select->value()-1,0x0A,val);
}
...
void JVlibForm::on_Part3_ReceivePrgChg_enable_toggled(bool val) {
  ...
  Part3_PatchGroup_select->setEnabled(val && AcceptBankSel_switch->isChecked());
  Part3_PatchNumber_select->setEnabled(val && AcceptProgramChg_switch->isChecked());
...
void JVlibForm::on_Part3_PatchGroup_select_currentIndexChanged(int val) {
 ...
 if (state_table->updates_enabled) {
  ...
  // update JV
  if (state_table->updates_enabled) {
    change_3(0xB0 + Part3_MidiChannel_select->value()-1, 0, MSB);
    change_3(0xB0 + Part3_MidiChannel_select->value()-1, 0x20, LSB);
    // Program Change - patch number is required for this
    change_2(0xC0 + Part3_MidiChannel_select->value()-1, Lval);
...
void JVlibForm::on_Part3_PatchNumber_select_valueChanged(int val) {
  ...
  if (state_table->updates_enabled) {
    ...
    if (state_table->jv_connect)
      change_2(0xC0 + Part3_MidiChannel_select->value()-1, pn);
    if (state_table->perf_mode) 
      if (CtlChl) SysControlRecvChannel_select->setValue(CtlChl);
    Part3_PatchName_display->setText(getPartPatchName(2));
  }	// end updates_enabled
