// part_5.cpp
// contains slots and functions for Parts Tab Part 5
// slots will set Part 5 displays to reflect a local control change and update active_area memory, then update the synth 
...
void JVlibForm::on_Part5_TestTone_switch_toggled(bool val) {
  unsigned char buf[6];
  if (val) {
  buf[0] = 0x90 + active_area->active_performance.perf_part[4].MIDI_channel;
  ...
  close_ports();
    ...
    buf[0] = 0xB0 + active_area->active_performance.perf_part[4].MIDI_channel;
    buf[1] = 0x7B;
    buf[2] = 0;
    buf[3] = 0xB0 + active_area->active_performance.perf_part[4].MIDI_channel;
    buf[4] = 0x79;
    buf[5] = 0;
  if (open_ports() == EXIT_FAILURE) return;
...
void JVlibForm::on_Part5_PatchGroup_select_currentIndexChanged(int val) {
 ...
 if (state_table->updates_enabled) {
  int MSB,LSB;
  unsigned char buf[8];
  int CtlChl = toggleControlChannel(5);
  ...
  int Hval = pn<128?0:1;
  int Lval = (pn%128);
  active_area->active_performance.perf_part[4].patch_num_high = Hval;
  active_area->active_performance.perf_part[4].patch_num_low = Lval;
  ...
  if (state_table->updates_enabled) {
    ...
    buf[6] = 0xC0+active_area->active_performance.perf_part[4].MIDI_channel;
    buf[7] = Lval;
    if (open_ports() == EXIT_FAILURE) return;
...
void JVlibForm::on_Part5_PatchNumber_select_valueChanged(int val) {
  if (active_area->active_performance.perf_part[4].patch_group) {
  ...
  if (state_table->updates_enabled) {
    ...
    int pn = val-1;
    unsigned char buf[2];
    active_area->active_performance.perf_part[4].patch_num_high = 0;
    active_area->active_performance.perf_part[4].patch_num_low = pn;
    if (state_table->jv_connect) {
      // update JV
      buf[0] = 0xC0 + active_area->active_performance.perf_part[4].MIDI_channel;
      buf[1] = pn;
      if (open_ports() == EXIT_FAILURE) return;
