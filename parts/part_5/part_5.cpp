...
//  Part5_Pan_select->setStatusTip(QString("Pan value: ")+QString::number(val));
  if (state_table->perf_mode) setPartSingleValue(4,7,val+64);
  if (state_table->GM_mode && state_table->updates_enabled) change_3(0xB0+Part5_MidiChannel_select->value()-1,0x0A,val+64);
}
