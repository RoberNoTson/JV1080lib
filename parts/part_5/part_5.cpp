...
void JVlibForm::on_Part5_ReceivePrgChg_enable_toggled(bool val) {
  if (state_table->perf_mode) {
    ...
    Part5_PatchGroup_select->setEnabled(val && AcceptBankSel_switch->isChecked());
    Part5_PatchNumber_select->setEnabled(val && AcceptProgramChg_switch->isChecked());
...
void JVlibForm::on_Part5_ReceiveVolume_enable_toggled(bool val) {
  if (state_table->perf_mode) setPartSingleValue(4,0xF,val);
}
void JVlibForm::on_Part5_ReceiveHold_enable_toggled(bool val) {
  if (state_table->perf_mode) setPartSingleValue(4,0x10,val);
}
...
void JVlibForm::on_Part5_PatchGroup_select_currentIndexChanged(int val) {
 ...
 if (state_table->updates_enabled) {
  ...
  // update JV
  if (state_table->updates_enabled) {
    change_3(0xB0 + Part5_MidiChannel_select->value()-1, 0, MSB);
    change_3(0xB0 + Part5_MidiChannel_select->value()-1, 0x20, LSB);
    // Program Change - patch number is required for this
    change_2(0xC0 + Part5_MidiChannel_select->value()-1, Lval);
...
void JVlibForm::on_Part5_PatchNumber_select_valueChanged(int val) {
  ...
  if (state_table->updates_enabled) {
    ...
    if (state_table->jv_connect)
      change_2(0xC0 + Part5_MidiChannel_select->value()-1, pn);
    if (state_table->perf_mode) 
      if (CtlChl) SysControlRecvChannel_select->setValue(CtlChl);
    Part5_PatchName_display->setText(getPartPatchName(4));
  }	// end updates_enabled
