...
void JVlibForm::on_Part11_ReceivePrgChg_enable_toggled(bool val) {
  if (state_table->perf_mode) {
    ...
    Part11_PatchGroup_select->setEnabled(val && AcceptBankSel_switch->isChecked());
    Part11_PatchNumber_select->setEnabled(val && AcceptProgramChg_switch->isChecked());
...
void JVlibForm::on_Part11_PatchGroup_select_currentIndexChanged(int val) {
 ...
 if (state_table->updates_enabled) {
  ...
  // update JV
  if (state_table->updates_enabled) {
    change_3(0xB0 + Part11_MidiChannel_select->value()-1, 0, MSB);
    change_3(0xB0 + Part11_MidiChannel_select->value()-1, 0x20, LSB);
    // Program Change - patch number is required for this
    change_2(0xC0 + Part11_MidiChannel_select->value()-1, Lval);
...
void JVlibForm::on_Part11_PatchNumber_select_valueChanged(int val) {
  ...
  if (state_table->updates_enabled) {
    ...
    int CtlChl=0;
    if (state_table->perf_mode) {
      CtlChl = toggleControlChannel(11);
    ...
    if (state_table->jv_connect)
      change_2(0xC0 + Part11_MidiChannel_select->value()-1, pn);
    if (state_table->perf_mode) 
      if (CtlChl) SysControlRecvChannel_select->setValue(CtlChl);
    Part11_PatchName_display->setText(getPartPatchName(10));
  }	// end updates_enabled
