...
//  Part2_Pan_select->setStatusTip(QString("Pan value: ")+QString::number(val));
  if (state_table->perf_mode) setPartSingleValue(1,7,val+64);
  if (state_table->GM_mode && state_table->updates_enabled) change_3(0xB0+Part2_MidiChannel_select->value()-1,0x0A,val+64);
}
...
//  Part2_SetPatchMax();
  ...
  active_area->active_performance.perf_part[1].patch_num_high = pn/16;
  active_area->active_performance.perf_part[1].patch_num_low = pn%16;
  ...
  buf[2] = 0x10 + 0x1;
  buf[3] = 0x02;
  memcpy((void *)&buf[4], (const void *)&active_area->active_performance.perf_part[1].patch_group,4);
  if (sysex_update((const unsigned char*)&buf,8) == EXIT_FAILURE) {
    puts("OOPS 2!"); return;
  }
  Part2_PatchName_display->setText(getPartPatchName(1));
  if (Patch_PerfPartNum_select->currentIndex()==1) {
...
void JVlibForm::on_Part2_PatchNumber_select_valueChanged() {
