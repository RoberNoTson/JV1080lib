...
void JVlibForm::on_Part2_TestTone_switch_toggled(bool val) {
  unsigned char buf[6];
  if (val) {
  buf[0] = 0x90 + active_area->active_performance.perf_part[1].MIDI_channel;
  ...
  close_ports();
    ...
    buf[0] = 0xB0 + active_area->active_performance.perf_part[1].MIDI_channel;
    buf[1] = 0x7B;
    buf[2] = 0;
    buf[3] = 0xB0 + active_area->active_performance.perf_part[1].MIDI_channel;
    buf[4] = 0x79;
    buf[5] = 0;
  if (open_ports() == EXIT_FAILURE) return;
...
void JVlibForm::on_Part2_PatchGroup_select_currentIndexChanged(int val) {
 ...
 if (state_table->updates_enabled) {
  ...
  if (state_table->updates_enabled) {
    ...
    buf[6] = 0xC0+active_area->active_performance.perf_part[1].MIDI_channel;
    buf[7] = Lval;
    if (open_ports() == EXIT_FAILURE) return;
...
void JVlibForm::on_Part2_PatchNumber_select_valueChanged(int val) {
  ...
  if (state_table->updates_enabled) {
    ...
    if (state_table->jv_connect) {
      // update JV
      buf[0] = 0xC0 + active_area->active_performance.perf_part[1].MIDI_channel;
      buf[1] = pn;
      if (open_ports() == EXIT_FAILURE) return;
