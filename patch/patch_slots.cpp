...
void JVlibForm::on_Patch_TestTone_switch_toggled(bool val) {
  if (state_table->jv_connect) {
  unsigned char buf[6];
  int pn = state_table->perf_mode ? (Patch_PerfPartNum_select->itemText(Patch_PerfPartNum_select->currentIndex()).toInt() - 1) : 0;
  if (val) {
    buf[0] = 0x90 + active_area->active_performance.perf_part[pn].MIDI_channel;
    ...
    buf[0] = 0xB0 + active_area->active_performance.perf_part[pn].MIDI_channel;
    buf[1] = 0x7B;
    buf[2] = 0;
    buf[3] = 0xB0 + active_area->active_performance.perf_part[pn].MIDI_channel;
    buf[4] = 0x79;
    buf[5] = 0;
  if (open_ports() == EXIT_FAILURE) return;
