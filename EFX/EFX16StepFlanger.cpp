// EFX16StepFlanger.cpp
#include        "JVlibForm.h"
#include        <QtGui>
#include        "EFX/EFX.h"

void JVlibForm::EFX16StepFlanger() {
  enable_4(true);
  enable_6(true);
  enable_7(true);
  enable_8(true);
  ...
  if (state_table->perf_mode && MainTabWidget->currentIndex()==1) {	// get Performance EFX settings
    ...
    EFXparm5_label->setText(QString("Step Rate"));
    PerfEFXparm5_select->setRange(0, 125);
  ...
  if (MainTabWidget->currentIndex()>1) {		// PatchEFX tab settings for either Perf or Patch
    ...
    PatchEFX_parm5_label->setText(QString("Step Rate"));
    PatchEFX_parm5_select->setRange(0, 125);
