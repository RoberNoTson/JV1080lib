...
void JVlibForm::EFX13Space_D() {
  ...
  if (SysMode_select->currentIndex()==0 && MainTabWidget->currentIndex()==1) {	// get Performance EFX settings
    ...
    EFXparm4_label->setText(QString("Phase "));
    PerfEFXparm4_select->setRange(0, 90);
  ...
  if (MainTabWidget->currentIndex()>1) {		// PatchEFX tab settings for either Perf or Patch
    ...
    PatchEFX_parm4_label->setText(QString("Phase "));
    PatchEFX_parm4_select->setRange(0, 90);
